...
@mcp.resource("shift-planner://team-member-property-values/matrix")
def team_member_property_values_matrix_resource() -> dict[str, Any]:
    """Return the org-wide team member properties matrix (definitions + per-member values)."""
    with db_session() as db:
        return list_property_values_matrix(
            db, organization_id=mcp_organization_id(), active_definitions_only=True
        ).model_dump(mode="json")
