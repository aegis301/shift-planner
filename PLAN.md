...
## Next Steps
1. R2 fairness accounts, the solver fixture (`comfortable` / `tight` / `infeasible` / `arbzg`), SolverRun persistence, the CP-SAT roster model (tier A), and solver controls in `/planning` plus MCP (#76) are in place. Next: write and implement ArbZG rest / weekly-average encodings (tier B) against the fourth fixture profile, then shift swaps (#77).
