...
## Next Steps
1. R2 fairness accounts and Analysis/picker deviation are in place. Solver fixture seed includes `comfortable`, `tight`, `infeasible`, and `arbzg`. SolverRun persistence and the Compose worker are in; the solve function is still a stub. Next: CP-SAT roster suggestions (R3) that replace the stub and can encode ArbZG rest and weekly-average rules against the fourth profile.
2. Run Alembic migrations against Postgres after pulling `202609210002` (`solver_runs` plus org solver time-budget ceiling). `202609210001` drops leftover global unique index `ix_doctors_email` on `team_members`.
