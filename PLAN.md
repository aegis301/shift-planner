...
## Next Steps
1. R2 fairness, the solver fixture, SolverRun persistence, the CP-SAT roster model (tier A), solver controls in `/planning` (#76), shift swap / giveaway requests (#77), the swap marketplace UI (#78), and swap UI states that name why an offer or the approval queue is unavailable (#99) are in place. Next: write and implement ArbZG rest / weekly-average encodings (tier B) against the fourth fixture profile.
