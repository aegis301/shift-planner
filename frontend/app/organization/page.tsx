...
export default function OrganizationAdminPage() {
    ...
    void (async () => {
      try {
        const [o, r, d] = await Promise.all([
          ...
          apiFetch<DoctorRow[]>("/api/v1/doctors")
        ]);
        setOrg(o);
        setRequests(r);
        setDoctors(d.filter((x) => x.user_id == null));
      } catch {
        setOrg(null);
      }
    })();
  ...
  async function approveCreate(req: JoinRequest, event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiFetch(`/api/v1/organization/join-requests/${req.id}/approve-create-doctor`, {
        method: "POST",
        body: JSON.stringify({
  ...
  async function approveLink(req: JoinRequest, doctorId: number) {
    setMsg("");
    try {
      await apiFetch(`/api/v1/organization/join-requests/${req.id}/approve-link-doctor`, {
        method: "POST",
  ...
  return (
    <div className="grid gap-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-semibold text-ink">{t(locale, "organizationAdminTitle")}</h1>
      </div>
      {org ? (
        <Card>
          ...
          {requests.map((req) => (
            <Card key={req.id}>
              ...
              <form className="mt-4 grid gap-3 border-t border-slate-100 pt-4" onSubmit={(e) => void approveCreate(req, e)}>
                <p className="text-sm font-semibold text-ink">{t(locale, "approveAsNewDoctor")}</p>
                ...
                <button
                  type="submit"
                  className="h-11 rounded-lg bg-ink px-4 text-sm font-semibold text-white"
                >
                  {t(locale, "confirmApproveCreate")}
                </button>
              </form>
              <div className="mt-4 border-t border-slate-100 pt-4">
                <p className="text-sm font-semibold text-ink">{t(locale, "linkExistingDoctor")}</p>
                <p className="mt-1 text-xs text-slate-500">{t(locale, "selectDoctorToLink")}</p>
