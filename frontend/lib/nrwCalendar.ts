...
export function slotTouchesWeekendOrNrwHoliday(
  slot: {
    slot_date: string;
  ...
  const start = slot.starts_at ? localDateKey(slot.starts_at, timeZone) : slot.slot_date;
  const end = slot.ends_at ? localDateKey(slot.ends_at, timeZone) : start;
