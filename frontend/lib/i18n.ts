export type Locale = "de" | "en";

export const dictionaries = {
  de: {
    appName: "Shift Planner",
    dashboard: "Dashboard",
    planning: "Planning",
    doctors: "Ärztinnen und Ärzte",
    shiftTypes: "Dienstarten",
    requests: "Wünsche",
    roster: "Dienstplan",
    validation: "Prüfung",
    exports: "Export",
    settings: "Einstellungen",
    login: "Anmelden",
    logout: "Abmelden",
    email: "E-Mail",
    password: "Passwort",
    adminLogin: "Admin Login",
    aiFirst: "AI-first mit FastMCP",
    currentFocus: "Aktueller Fokus",
    mvpFocus: "Monatliche Dienstplanung für Ärztinnen und Ärzte mit manueller Planung und Regelwarnungen.",
    freshStart: "Frischer Start",
    noData: "Noch keine Daten",
    addDoctor: "Arzt hinzufügen",
    addShiftType: "Dienstart hinzufügen",
    addRequest: "Wunsch erfassen",
    addAssignment: "Dienst eintragen",
    name: "Name",
    employment: "Stellenumfang",
    notes: "Notizen",
    save: "Speichern",
    code: "Code",
    germanName: "Deutscher Name",
    englishName: "Englischer Name",
    start: "Beginn",
    end: "Ende",
    category: "Kategorie",
    planningPeriod: "Planungsmonat",
    year: "Jahr",
    month: "Monat",
    doctorId: "Arzt-ID",
    shiftTypeId: "Dienstart-ID",
    date: "Datum",
    requestType: "Wunschtyp",
    wish: "Wunsch",
    noGo: "No-Go",
    preference: "Präferenz",
    warnings: "Warnungen",
    csvExport: "CSV Export",
    printView: "Druckansicht",
    language: "Sprache",
    mcpStatus: "MCP Status",
    mcpText: "Alle Kernaktionen laufen über Backend-Services, damit REST und FastMCP dieselbe Logik nutzen.",
    open: "Öffnen",
    refresh: "Aktualisieren",
    signedIn: "Angemeldet",
    notSignedIn: "Nicht angemeldet",
    loginFailed: "Anmeldung fehlgeschlagen",
    created: "Gespeichert",
    apiUnavailable: "API nicht erreichbar",
    id: "ID",
    createPeriod: "Planungsmonat anlegen",
    createAndLoadPeriod: "Monat anlegen und laden",
    day: "Tagdienst",
    night: "Nachtdienst",
    onCall: "Rufdienst",
    other: "Andere",
    yes: "Ja",
    no: "Nein",
    emptyValue: "—",
    isActive: "Aktiv",
    createdAt: "Erstellt am",
    status: "Status",
    note: "Notiz",
    requestDate: "Wunschdatum",
    assignmentDate: "Zuweisungsdatum",
    manualOverride: "Manuelle Überschreibung",
    severity: "Schweregrad",
    validationMessage: "Meldung",
    details: "Details",
    assignmentId: "Zuweisungs-ID",
    requestRecordId: "Wunsch-ID",
    planningPeriodId: "Planungsmonat-ID",
    matrix: "Matrix",
    matrixEditor: "Planungsmatrix",
    matrixHelp: "Tage als Zeilen, Teammitglieder als Spalten. Jede Zelle hat einen Status und optional einen Kommentar.",
    periodId: "Planungsmonat-ID",
    loadMatrix: "Matrix laden",
    cellComment: "Kommentar",
    sourceEmail: "Quell-E-Mail",
    summary: "Zusammenfassung",
    doctorPeriodNotes: "Monatsnotizen",
    dienstwunsch: "Dienstwunsch",
    urlaub: "Urlaub",
    keinDienst: "Kein Dienst",
    forschung: "Forschung",
    lehre: "Lehre",
    frei: "Frei",
    tagdienst: "Tagdienst",
    nachtdienst: "Nachtdienst",
    spaetdienst: "Spätdienst",
    rufdienst: "Rufdienst",
    clear: "Leeren",
    close: "Schließen",
    saved: "Gespeichert",
    matrixCsvExport: "Matrix CSV Export",
    rosterCsvExport: "Dienstplan CSV Export",
    finalRosterMatrix: "Finaler Dienstplan",
    finalRosterHelp: "Tage als Zeilen, Dienstarten als Spalten. Jede Zelle weist einen Dienst einer Person zu und zeigt Konflikte mit den Wünschen farbig an.",
    loadRoster: "Dienstplan laden",
    noShiftTypesForRoster: "Noch keine aktiven Dienstarten. Lege zuerst Dienstarten an, damit Spalten im Dienstplan entstehen.",
    conflict: "Konflikt",
    planningWorkspaceHelp: "Ein Planungsmonat steuert Wünsche, finalen Dienstplan, Exporte, Prüfung und Arbeitslast.",
    selectedMonth: "Ausgewählter Monat",
    wishesSection: "Wünsche",
    rosterSection: "Finaler Dienstplan",
    wishesCsvExport: "Wünsche CSV Export",
    conflictSummary: "Konfliktübersicht",
    noConflicts: "Keine Konflikte",
    workloadStats: "Arbeitslast",
    unassignedSlots: "Offene Dienste",
    totalShifts: "Gesamt",
    conflicts: "Konflikte",
    noPlanningPeriodSelected: "Noch kein Planungsmonat ausgewählt.",
    autosaved: "Automatisch gespeichert",
    saving: "Speichert...",
    saveNow: "Jetzt speichern",
    saveNowHint: "Alle Änderungen werden automatisch gespeichert. Diese Aktion lädt die Matrix neu."
  },
  en: {
    appName: "Shift Planner",
    dashboard: "Dashboard",
    planning: "Planning",
    doctors: "Doctors",
    shiftTypes: "Shift types",
    requests: "Requests",
    roster: "Roster",
    validation: "Validation",
    exports: "Export",
    settings: "Settings",
    login: "Log in",
    logout: "Log out",
    email: "Email",
    password: "Password",
    adminLogin: "Admin login",
    aiFirst: "AI-first with FastMCP",
    currentFocus: "Current focus",
    mvpFocus: "Monthly doctor scheduling with manual planning and rule warnings.",
    freshStart: "Fresh start",
    noData: "No data yet",
    addDoctor: "Add doctor",
    addShiftType: "Add shift type",
    addRequest: "Record request",
    addAssignment: "Add assignment",
    name: "Name",
    employment: "Employment",
    notes: "Notes",
    save: "Save",
    code: "Code",
    germanName: "German name",
    englishName: "English name",
    start: "Start",
    end: "End",
    category: "Category",
    planningPeriod: "Planning month",
    year: "Year",
    month: "Month",
    doctorId: "Doctor ID",
    shiftTypeId: "Shift type ID",
    date: "Date",
    requestType: "Request type",
    wish: "Wish",
    noGo: "No-go",
    preference: "Preference",
    warnings: "Warnings",
    csvExport: "CSV export",
    printView: "Print view",
    language: "Language",
    mcpStatus: "MCP status",
    mcpText: "Core actions run through backend services so REST and FastMCP use the same logic.",
    open: "Open",
    refresh: "Refresh",
    signedIn: "Signed in",
    notSignedIn: "Not signed in",
    loginFailed: "Login failed",
    created: "Saved",
    apiUnavailable: "API unavailable",
    id: "ID",
    createPeriod: "Create planning month",
    createAndLoadPeriod: "Create and load month",
    day: "Day shift",
    night: "Night shift",
    onCall: "On-call",
    other: "Other",
    yes: "Yes",
    no: "No",
    emptyValue: "—",
    isActive: "Active",
    createdAt: "Created at",
    status: "Status",
    note: "Note",
    requestDate: "Request date",
    assignmentDate: "Assignment date",
    manualOverride: "Manual override",
    severity: "Severity",
    validationMessage: "Message",
    details: "Details",
    assignmentId: "Assignment ID",
    requestRecordId: "Request ID",
    planningPeriodId: "Planning period ID",
    matrix: "Matrix",
    matrixEditor: "Planning matrix",
    matrixHelp: "Days as rows, team members as columns. Each cell has one status and an optional comment.",
    periodId: "Planning period ID",
    loadMatrix: "Load matrix",
    cellComment: "Comment",
    sourceEmail: "Source email",
    summary: "Summary",
    doctorPeriodNotes: "Monthly notes",
    dienstwunsch: "Duty wish",
    urlaub: "Vacation",
    keinDienst: "No duty",
    forschung: "Research",
    lehre: "Teaching",
    frei: "Free",
    tagdienst: "Day duty",
    nachtdienst: "Night duty",
    spaetdienst: "Late duty",
    rufdienst: "On-call",
    clear: "Clear",
    close: "Close",
    saved: "Saved",
    matrixCsvExport: "Matrix CSV export",
    rosterCsvExport: "Roster CSV export",
    finalRosterMatrix: "Final roster",
    finalRosterHelp: "Days are rows, shift types are columns. Each cell assigns one shift to one person and highlights conflicts with requests.",
    loadRoster: "Load roster",
    noShiftTypesForRoster: "No active shift types yet. Create shift types first so the roster has columns.",
    conflict: "Conflict",
    planningWorkspaceHelp: "One planning month controls requests, final roster, exports, validation, and workload.",
    selectedMonth: "Selected month",
    wishesSection: "Requests",
    rosterSection: "Final roster",
    wishesCsvExport: "Requests CSV export",
    conflictSummary: "Conflict summary",
    noConflicts: "No conflicts",
    workloadStats: "Workload",
    unassignedSlots: "Unassigned shifts",
    totalShifts: "Total",
    conflicts: "Conflicts",
    noPlanningPeriodSelected: "No planning month selected yet.",
    autosaved: "Autosaved",
    saving: "Saving...",
    saveNow: "Save now",
    saveNowHint: "All changes are saved automatically. This action reloads the matrix."
  }
} as const;

export type TranslationKey = keyof typeof dictionaries.de;

export function t(locale: Locale, key: TranslationKey): string {
  return dictionaries[locale][key];
}
