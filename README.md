...
- `GET /api/v1/auth/me` (and login/register responses) returns `organization_id`, nested `organization` (`id`, `name`, **`slug`**, `plan_tier`), `team_member_id`, `shift_groups` (linked team member’s groups, if any), `planner_shift_groups` (planner role), **`organization_shift_groups`** (all shift groups in the active org when **`role`** is **`admin`**, otherwise `[]`), and `capabilities` (`admin`, `planning`, `team_member_portal`) for the **active membership** only so the frontend can show Dashboard, Planning, My planning, Profile, admin team/shift sections, and the current organization in the shell and settings. After switching org, navigation is aligned with those capabilities (same default routes as after login).
