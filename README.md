...
When an organization has an active **work-time rule set**, month validation and assignment preflight also evaluate statutory rules over `PlanState` using **statutory** minutes (`TimeEntry.statutory_minutes` or `statutory_work_minutes`), never tariff credit. Codes: **`WORKTIME_MAX_DAILY`**, **`WORKTIME_MIN_REST`**, **`WORKTIME_REST_COMPENSATION_PENDING`**, **`WORKTIME_REST_AFTER_LONG_DUTY`**, **`WORKTIME_WEEKLY_AVERAGE`**, **`WORKTIME_WEEKLY_AVERAGE_OPT_OUT`**, **`WORKTIME_CONSECUTIVE_DAYS`**, **`WORKTIME_MAX_DUTIES`**, **`WORKTIME_DOCUMENTATION_GAP`**. Weekly-average lookback uses pre-aggregated per-day statutory totals rather than a year of roster slots. **`opt_out_weekly_cap`** resolves **`applicable_weekly_cap`** per date from the **`WorkTimeConsent`** register (base tier without effective consent; signed opt-out tier until `effective_until` after revocation). **`error`** severity blocks roster assignment at preflight.
