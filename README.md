...
Pull request CI checks out the PR and **merges the latest target branch** (`main`) before lint, typecheck, tests, and the container smoke job, so two independently green branches cannot hide a combined type error. After each push to `main`, [.github/workflows/refresh-pr-ci.yml](.github/workflows/refresh-pr-ci.yml) re-runs CI on open same-repo PRs so those merges happen again against the new `main`. That still races if someone merges a PR before the re-run finishes: in GitHub → Settings → Branches (or Rulesets) protect `main` with required checks (`backend`, `mcp`, `frontend`, `container-smoke`) and **Require branches to be up to date before merging**. A **merge queue** is even stronger; CI also listens for `merge_group` so queue entries are tested.
...
Mutating MCP tools require a short-lived MCP JWT (org, role, shift groups) or `MCP_ADMIN_TOKEN` (admin break-glass). **Read resources and read tools also require `access_token`.** Local CLI runs use FastMCP stdio by default; Docker Compose sets `MCP_TRANSPORT=http`. The in-app assistant acts as the signed-in membership. `MCP_ORGANIZATION_ID` still applies only to the break-glass admin token (otherwise `DEFAULT_ORGANIZATION_ID`). Do not publish MCP on the public internet in production (`docker-compose.prod.yml` exposes MCP only on the internal network).

Org admins configure a BYOK model key under **Team → Organisation**. Planning Analysis runs three fixed AI tasks (wishes summary, validation explanation, fair-roster draft with confirm-to-apply). Optional operator-only Langfuse: `docker compose -f docker-compose.observability.yml up` (see [deploy/README.md](deploy/README.md)); leave `LANGFUSE_HOST` empty to disable tracing.
...
**Working hours / timesheets:** Admins define **worker groups** (`GET|POST|PATCH|DELETE /api/v1/worker-groups`) with weekly hours and vacation days at 100%, category credit rules, and day-status mappings. Dated **employment periods** and **opening balances** live under `/api/v1/hours/members/{id}/employment-periods` and `/opening`. Time entries and computed timesheets: `/api/v1/hours/members/{id}/entries`, `/timesheet`, `/timesheet.csv`, fill-from-roster, fill-regular-week. Planner UI `/hours`; team member `/my-hours` (read-only). MCP: `shift-planner://worker-groups`, `shift-planner://hours/timesheet/{id}/{year}/{month}`, plus admin tools.
