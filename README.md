...
- `GET /api/v1/auth/me` (and login/register responses) returns `organization_id`, nested `organization` (`id`, `name`, **`slug`**, `plan_tier`), `team_member_id`, `shift_groups`, `planner_shift_groups`, and `capabilities` (`admin`, `planning`, `team_member_portal`) so the frontend can show Dashboard, Planning, My planning, Profile, admin team/shift sections, and the current organization in the shell and settings.
