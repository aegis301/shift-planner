...
- Planning status endpoints (require `shift_group_id` query param): `POST /api/v1/planning-periods/{id}/draft`, `POST /api/v1/planning-periods/{id}/preliminary`, `POST /api/v1/planning-periods/{id}/publish`. Legacy `POST /api/v1/planning-periods/{id}/unpublish` maps to `preliminary`. `GET /api/v1/planning-periods` includes `shift_group_statuses[]` per month.
