...
OR-Tools is a backend runtime dependency (same image as the solver-worker). Rebuild the backend and worker images after pulling this change so `ortools` is installed in the container; bind mounts do not install new Python packages.
...
python -m app.scripts.seed_work_time_presets
...
Planning screens read server state through TanStack Query (`frontend/lib/queryKeys.ts`, `frontend/lib/queries/`). The client is created in `frontend/app/ClientRoot.tsx` (30 second stale time, one retry, no retry on 401 or 403). React Query Devtools render in development. Switching the active organization clears the cache.
...
Use `/planning` for the planner workflow and `/my-planning` for linked team members. One selected planning month controls wishes, final roster assignment, inline validation, CSV exports, workload stats, the **compliance report**, **fairness accounts**, and **solver generate / apply** on the Analysis tab. The planner page supports both a full stacked view and a tabbed view for Wishes, Roster, and Analysis.
...
**Time entries** (`GET|POST /api/v1/time-entries`, `PATCH|DELETE /api/v1/time-entries/{id}`, reconciliation, `POST .../derive`, `GET .../ledger`) store the dated work/absence ledger, including **`statutory_minutes`** and **`credited_minutes`**. Roster and day-status rows are derived; members manage their own entries (`team_member_portal=true`). The ledger read returns opening balance, contract-target minutes, separate statutory/credited totals, a running overtime account, and entries (`derived_snapshot` stays visible after corrections). UI: **`/my-hours`** (linked team members) and **`/hours`** (planners/admins, `shift_group_id` required for planners). MCP: `shift-planner://time-entries/{team_member_id}`, `get_hours_ledger_tool`, and token-gated upsert/derive. Dual valuation (`work_time_valuation.py`) computes both minute figures from contract-group category rules, with an optional per-template `valuation_override`.

**Duty activity** (`GET|POST /api/v1/duty-activity`, `DELETE /api/v1/duty-activity/{id}`, `GET /api/v1/duty-activity/utilization/{planning_period_id}`) records Rufbereitschaft **`call_out`** and Bereitschaftsdienst **`in_duty_activity`** episodes on the same ledger (`source` `duty_activity`). Both require `roster_slot_id`, `started_at`, and `ended_at` inside the assigned slot; overlapping episodes on one slot are rejected. A call-out stores statutory minutes and `counts_toward_contract` from the contract-group `call_outs_count_as_work` flag; in-duty activity adds no statutory time and feeds utilization. Per-slot and per-template aggregates return `worked_minutes / duty_minutes`, the tariff band from the active rule set’s `duty_utilization_bands`, an explicit `exceeds_on_call_threshold` flag, and **coverage** (recorded duties / all assigned duties). Linked members manage their own episodes after acknowledging the org purpose statement (`GET|POST /api/v1/duty-activity/purpose`). Start may omit `ended_at` (running episode); `PATCH /api/v1/duty-activity/{id}` sets `ended_at` and optional reason. One-tap capture lives on `/my-planning` and the team-member dashboard; retrospective entry is on the My shifts tab. `GET /api/v1/duty-activity/slots/{id}/utilization` returns the rule-set band for the assigned member. Individual episode reads for any other member default to **403** unless `DutyActivityAccessPolicy.individual_read_roles` grants `admin` and/or `planner`; every successful individual read and every policy change is audited. Retention purge: `python -m app.scripts.purge_duty_activity` (or MCP `purge_duty_activity_episodes_tool`). Works-council export (`GET /api/v1/exports/duty-activity/works-council/{planning_period_id}.xlsx|.pdf`) is aggregate-only with small-group suppression. Admin UI: **Team** → **Diensttätigkeit**. Linked members see the purpose card on `/profile`. MCP: `shift-planner://duty-utilization/{planning_period_id}`, `shift-planner://duty-activity-access-policy`, `shift-planner://duty-activity/works-council/{planning_period_id}`, token-gated `record_duty_activity_tool`, `update_duty_activity_tool`, `update_duty_activity_access_policy_tool`, `purge_duty_activity_episodes_tool`. Alembic `202609110003`.

**Work-time rule sets** (`GET|POST|PATCH|DELETE /api/v1/work-time-rule-sets`) store versioned statutory configuration (`WorkTimeRuleSet`). Only one set is active per organization; editing a set referenced by a plan snapshot creates a new version. Plan versions store `work_time_rule_set_version_id`. Seed presets (`ArbZG-Grundmodell`, `TV-Ärzte (TdL)`, `TV-Ärzte (VKA)`) live in `work_time_rule_set_presets`; `POST /api/v1/work-time-rule-sets/presets/{code}/adopt` copies one into the org (and TdL/VKA also seed Stufe contract groups). Admin UI: **Team** → **Arbeitszeitregeln**. MCP: `shift-planner://work-time-rule-sets`, `shift-planner://work-time-rule-set-presets`, and token-gated create/update/delete/adopt. Rule evaluation is not in this layer.
...
- `requires_coupled_shift` (requires `paired_shift_variant_id`, optional `partner_day_offset` default `1`, range `-7`..`7`; same person must also be assigned to the partner variant on the offset calendar day, including when that day is in the next planning month)
...
These constraints are evaluated for assignment preflight and validation warnings through a shared backend rule engine over a date window (`PlanState`), so rest and coupling also see assignments in the adjacent month. **`error`** severity blocks roster assignment at preflight. **Global rule:** roster assignments are blocked when the shift overlaps any wishes-matrix day whose status has `blocks_roster_assignment` (`ROSTER_MATRIX_UNAVAILABLE_OVERLAP`), unless a template/variant sets `unavailable_overlap_policy` to `allow` or `warn`.
...
**Team member properties (competencies):** Admins define custom fields per organization at `GET|POST|PATCH|DELETE /api/v1/team-member-property-definitions` (types: `number`, `date`, `select`, `multi_select`, `text`; options required for select types). Values are stored per team member at `GET|PUT /api/v1/team-members/{id}/property-values`; the admin-only `GET /api/v1/team-member-property-matrix` and MCP resource `shift-planner://team-member-property-matrix` return the org-wide sparse matrix. Type-aware, AND-combined server-side filtering uses `POST /api/v1/team-member-property-matrix/search` or the read-only MCP `filter_team_member_property_matrix_tool`. Each definition may set `editable_by_team_member` so linked members can maintain their own values on `/profile` while admins edit all fields in the staff directory. UI: Organization → Team → Properties opens an inline-editable, filterable values table with a separate field-definitions tab; values also appear in the team member editor and profile.
...
When an organization has an active **work-time rule set**, month validation and assignment preflight also evaluate statutory rules over `PlanState` using **statutory** minutes (`TimeEntry.statutory_minutes` or `statutory_work_minutes`), never tariff credit. Codes: **`WORKTIME_MAX_DAILY`**, **`WORKTIME_MIN_REST`**, **`WORKTIME_REST_COMPENSATION_PENDING`**, **`WORKTIME_REST_AFTER_LONG_DUTY`**, **`WORKTIME_WEEKLY_AVERAGE`**, **`WORKTIME_WEEKLY_AVERAGE_OPT_OUT`**, **`WORKTIME_CONSECUTIVE_DAYS`**, **`WORKTIME_MAX_DUTIES`**, **`WORKTIME_DOCUMENTATION_GAP`**. **`WORKTIME_MAX_DUTIES`** counts only the configured template **`categories`** (default **`bereitschaftsdienst`**). Weekly-average lookback uses pre-aggregated per-day statutory totals rather than a year of roster slots. **`opt_out_weekly_cap`** resolves **`applicable_weekly_cap`** per date from the **`WorkTimeConsent`** register (base tier without effective consent; signed opt-out tier until `effective_until` after revocation). **`error`** severity blocks roster assignment at preflight.

**Compliance report** (`GET /api/v1/compliance-report/{planning_period_id}`, XLSX/PDF under `/api/v1/exports/compliance-report/{id}.xlsx|.pdf`) is built from the same `evaluate_plan_state` findings and rule-layer metrics (statutory vs tariff credit, rolling weekly average and cap source, rest/compensation, consecutive days, duty counts, documentation coverage). The active rule-set **name and version** and a generation timestamp are printed on every export. Admins may omit `shift_group_id`; planners must pass one of their groups. Planning workspace Analysis tab. MCP: `shift-planner://compliance-report/{planning_period_id}`.
...
**Solver runs** (`POST /api/v1/planning-periods/{id}/solver-runs`, `GET .../solver-runs`, `GET .../solver-runs/{run_id}`, `POST .../solver-runs/{run_id}/apply`, `POST .../solver-runs/{run_id}/cancel`) queue asynchronous roster generation. The request returns immediately with a run id. A Compose **solver-worker** (`python -m app.scripts.solver_worker`) claims `queued` rows. Parameters always include `num_search_workers` (default `1`) and `random_seed`. Optional `objective_weights` on create override org defaults for that run only (`GET /api/v1/organization/solver-config` returns ceiling, default budget, and weights). The worker builds an OR-Tools CP-SAT model from rule `to_cpsat` and service eligibility masks (period roster, cover-template); `evaluate()` is not used to construct variables. No-gos are hard and are not an objective term. Unfilled slots use slack. Apply writes ordinary roster assignments through the existing upsert path and is never automatic. Published shift groups return **409**. Planning workspace: generate from the toolbar, poll, inspect breakdown / unfilled slots / post-check findings, apply behind confirmation. MCP: `shift-planner://solver-runs/{planning_period_id}/shift-group/{shift_group_id}`, `shift-planner://solver-runs/{planning_period_id}/run/{run_id}`, `run_roster_solver_tool`, `apply_solver_run_tool`, `cancel_solver_run_tool`. Alembic `202609210002`, `202609210003`.

**Shift swaps** (`POST /api/v1/shift-swaps`, `GET .../shift-swaps`, `GET .../shift-swaps/unresolved`, `GET .../shift-swaps/{id}`, `POST .../{id}/open|claim|accept|decline|withdraw|approve|reject|apply`, `GET .../shift-swaps/eligible-members`) persist giveaway and direct 1:1 exchange requests. Members offer a duty they currently hold; opening a giveaway publishes it to the group, opening a direct request targets a named member. Claiming a giveaway is atomic so exactly one concurrent claim wins. Eligibility is the same mask the solver uses; legality overlays the hypothetical post-swap `PlanState` and runs the rule registry (`error` refuses and names the violation, `warning` is stored for the approver). Apply writes ordinary roster assignments (source `shift_swap`) and snapshots a new plan version (`trigger` `swap_apply`) even when the group is published. Linked members offer from **My shifts** or an own roster cell, claim open giveaways, and track request status on `/my-planning`. A missing shift group, an unlinked account, and a draft plan each name the fix; draft offer buttons stay disabled and past slots stay hidden. Planners see unresolved `open`/`targeted` requests (duty date, days until duty, request age) and the approval queue on the Analysis tab of `/planning`. An empty queue is distinct from a missing shift group, and planners who are also team members get a link to `/my-planning`. Eligible partners are ranked by fairness deviation when accounts can be built. MCP: `shift-planner://shift-swaps/{planning_period_id}/shift-group/{shift_group_id}`, `.../unresolved`, `shift-planner://shift-swaps/request/{request_id}`, plus token-gated transition tools. Alembic `202609220001`.
...
The `/planning` toolbar includes destructive month actions behind confirmation dialogs: deleting a planning month removes its wishes, notes, generated roster slots, and assignments; regenerating a month clears roster assignments and rebuilds roster slots from the current shift templates. **Generate roster** queues a CP-SAT run (hidden for **published** shift groups); applying the proposal is a second confirmation and a normal roster write. Planning months use a **per-shift-group** 3-state lifecycle: `draft`, `preliminary`, `published` (stored in `planning_period_shift_group_statuses`). Publishing one Dienstgruppe does not change another group's status. Wishes day cells and month notes are also scoped by `shift_group_id`. Team members can read roster data for a group in `preliminary` and `published`; wishes and feedback are writable for that group while it is `draft` or `preliminary`. **Published** plans are read-only for planners and admins on the usual roster/wishes write paths; reopen to `preliminary` to edit (working version bumps, e.g. `1.0` → `1.1`). Applying an approved shift swap is the exception: it writes assignments and snapshots a new version without reopening. **Plan versions** snapshot roster + wishes per shift group (`0.1` on first preliminary share, `1.0` on first publish); use **Save version** while preliminary or browse **Versions** history.
...
Query parameter `shift_group_id`: **admins** may omit it for a full-org matrix, matrix notes, roster, validation, fairness accounts, and CSV exports. **Planners** (non-admin) must pass `shift_group_id` on `GET /api/v1/matrix/{id}`, matrix note routes under `/api/v1/matrix/{id}/notes`, `GET /api/v1/roster-matrix/{id}`, `GET /api/v1/validation/{id}`, `GET /api/v1/fairness/{id}`, and on both CSV export routes; the value must be one of their `user_shift_groups`. For roster XLSX/PDF exports, planners follow the same shift-group scope rules, while team-member portal users must pass `shift_group_id` and are checked against team-member shift-group membership. Creating or deleting a planning month (`POST` / `DELETE /api/v1/planning-periods`) is **admin-only**; status transitions, `POST /api/v1/planning-periods/{id}/sync-roster` (add/remove/update template slots while preserving assignments on unchanged slots), and destructive `POST /api/v1/planning-periods/{id}/regenerate-roster` remain available to any user with planning access (admin or planner).

Shift groups are managed at `GET|POST|PATCH|DELETE /api/v1/shift-groups` (`GET` for any planning user, scoped for planners; create/update/delete and membership `PUT`s are admin-only). Membership is **dated**: `PUT /api/v1/shift-groups/{id}/memberships` takes `{ "memberships": [ { "team_member_id", "start_date", "end_date" | null } ] }` (`null` end date = open-ended, several non-overlapping periods per person for rotations, `400` on overlap). The simpler `PUT /api/v1/shift-groups/{id}/team-members` with `team_member_ids` still works and is interpreted as "active today" (adds start today, removes end today). Reads return `team_member_ids` (active today) plus `team_member_memberships` with the dates; `TeamMember` rows carry `shift_group_ids` and `shift_group_memberships`. Roster assignments require the assignee to be on that month's period roster for a group covering the slot’s template.
