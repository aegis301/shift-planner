...
def _export_matrix_csv_from_read(matrix) -> str:
    cells = {(cell.cell_date, cell.team_member_id): cell for cell in matrix.cells}
    buffer = StringIO()
    writer = csv.writer(buffer)
    writer.writerow(["date", *[_planning_member_label(m) for m in matrix.team_members]])
...
def _export_roster_matrix_csv_from_read(matrix) -> str:
    slots_by_day: dict[date, list[Any]] = {}
    for slot in matrix.slots:
        slots_by_day.setdefault(slot.slot_date, []).append(slot)
    assignments = {assignment.roster_slot_id: assignment for assignment in matrix.assignments}
        ...
        for slot in slots_by_day.get(day.date, []):
            ...
            writer.writerow(
                [
                    ...
                    slot.template_code or "",
                    slot.variant_label or "",
                    getattr(slot, "category", None) or "",
                ]
            )
    return buffer.getvalue()


def build_roster_export_table_by_template_from_matrix(matrix) -> RosterExportTable:
    template_by_id = {template.id: template for template in matrix.shift_templates}
    columns: list[RosterExportColumn] = []
    column_index: dict[tuple[int | None, str | None, str | None], int] = {}
    for template in matrix.shift_templates:
    ...
    for slot in slots_sorted:
        ...
        idx = column_index.get(signature)
        if idx is None:
            template = template_by_id.get(slot.shift_template_id or -1)
            signature = (slot.shift_template_id, slot.template_code, slot.label)
            idx = column_index.get(signature)
            if idx is None:
                idx = len(columns)
                column_index[signature] = idx
                columns.append(
                    RosterExportColumn(
                        key=f"slot-{slot.id}",
                        title=_template_column_title(template, slot),
                    )
                )
        slot_to_column[slot.id] = idx
...
def export_version_matrix_csv(
    ...
    *,
    ...
    matrix = get_plan_version_matrix(
        db,
        version_id=version_id,
        planning_period_id=planning_period_id,
        organization_id=organization_id,
    )
    return _export_matrix_csv_from_read(matrix)


def export_version_roster_matrix_csv(
    db: Session,
    planning_period_id: int,
    *,
    version_id: int,
    ...
    matrix = get_plan_version_roster(
        db,
        version_id=version_id,
        planning_period_id=planning_period_id,
        organization_id=organization_id,
    )
    return _export_roster_matrix_csv_from_read(matrix)


def export_version_roster_matrix_xlsx(
    db: Session,
    planning_period_id: int,
    *,
    version_id: int,
...
def export_version_roster_matrix_pdf(
    db: Session,
    planning_period_id: int,
    *,
    version_id: int,
